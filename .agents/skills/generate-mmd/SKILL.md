...
1. Always use `key: C4` unless the user explicitly specifies a different key.
2. If $ARGUMENTS contains a style hint (e.g. "waltz", "slow ballad", "upbeat"), use it to pick bpm and rhythm.
3. Write 2–3 sections ([A], [B], optionally [C]), 4–6 measures each.
4. **Beat-count verification pass (mandatory):** Before writing the file, go through every measure line one by one and calculate its beat total:
   - Count bare digits outside `<…>` as 1 beat each
   - Count digits inside `<…>` as 0.5 beat each
   - Count each `-` as 1 beat
   - Count each `x` as 1 beat
   - The sum must equal `timeSignatureTop` (default 4). If it doesn't, fix the line before continuing.
